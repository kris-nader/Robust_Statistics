...
simulate_precision<-function(n,p,M, bool.contaminated = 1) {
  ...
  Theta1sparse <- zapsmall(Theta1)
  Theta1sparse_log = Theta1sparse==0
  FP_den<-sum(Theta1sparse_log)
  FN_den<-p*p-FP_den
  ...
  for (i in 1:M) {
    ...
    if (bool.contaminated == 1) {
      indices <- sample.int(p*n,p*n*0.1) # We need to contaminate 10 percent of the dataset
      for (j in indices) {
        X[j] <- rnorm(1, mean = 10, sd = sqrt(0.2))
      }
    }
    ...
    FP[i,1] <- sum(sample.theta[which(Theta1sparse_log)]!=0)/FP_den
    FN[i,1] <- sum(sample.theta[which(!Theta1sparse_log)]==0)/FN_den
    # print("three")
    #twostep quadrant
    # print("three.one")
    ...
    # print("four")
    FP[i,2] <- sum(tws.quad.theta[which(Theta1sparse_log)]!=0)/FP_den
    FN[i,2] <- sum(tws.quad.theta[which(!Theta1sparse_log)]==0)/FN_den
    ...
    # print("five")
    FP[i,3] <- sum(ths.quad.theta[which(Theta1sparse_log)]!=0)/FP_den
    FN[i,3] <- sum(ths.quad.theta[which(!Theta1sparse_log)]==0)/FN_den
    #twostep spearman
    ...
    # print("six")
    FP[i,4] <- sum(tws.sp.theta[which(Theta1sparse_log)]!=0)/FP_den
    FN[i,4] <- sum(tws.sp.theta[which(!Theta1sparse_log)]==0)/FN_den
    #threestep spearman
    ...
    # print("seven")
    FP[i,5] <- sum(ths.sp.theta[which(Theta1sparse_log)]!=0)/FP_den
    FN[i,5] <- sum(ths.sp.theta[which(!Theta1sparse_log)]==0)/FN_den
    #Gaussian
    ...
    if (p<n) {
      ...
      FP[i,7] <- sum(S.inv[which(Theta1sparse_log)]!=0)/FP_den
      FN[i,7] <- sum(S.inv[which(!Theta1sparse_log)]==0)/FN_den
    }
    # print("nine")
    if (i%%5==0) {
      print(i)
  ...
  KL_avg <- colMeans(sample.results)
  FP_avg <- colMeans(FP)
  FN_avg <- colMeans(FN)
  return(rbind(KL_avg,FP_avg,FN_avg))
}

# avg.results0pc1<-simulate_precision(50,3,100,0)
# ZeroDiff1<-(avg.results0pc1-c(0.23,0.59,0.33,0.29,0.27,0.27,0.14))
# ZeroMean1<-mean(ZeroDiff1)
# ZeroMean1
# avg.results0pc1
# avg.results0pc2<-simulate_precision(50,100,100,0)
# ZeroDiff2<-(avg.results0pc2-c(38,38.7,49.54,38.62,39.47,38.62))
# ZeroMean2<-mean(ZeroDiff2)
# ZeroMean2


# HunDiff<-avg.results10pc-c(145.56,55.54,71.02,55.66,57.01,55.49)
# HunMean<-mean(HunDiff)
avg.results10pc<-simulate_precision(50,100,100,1)
write.csv(avg.results10pc,file="sim_50_100_100_1.csv")

avg.results10pc<-simulate_precision(50,100,100,0)
write.csv(avg.results10pc,file="sim_50_100_100_0.csv")

avg.results10pc<-simulate_precision(50,5,100,1)
write.csv(avg.results10pc,file="sim_50_5_100_1.csv")

avg.results10pc<-simulate_precision(50,5,100,0)
write.csv(avg.results10pc,file="sim_50_5_100_0.csv")
